...
def test_slippery_score_short_opener():
    text = (
        "Plans break. The schedule was solid Monday. By Wednesday everything changed."
    )
...
def test_detect_triggers_finds_fear():
    triggers = detect_triggers(
        "Firefighting is costing you money. The risk of inaction is real."
    )
    names = [t["name"] for t in triggers]
...
def test_extract_concept_constraints():
    concept = extract_concept(
        "Constraints drive your schedule. Bottleneck management improves throughput."
    )
...
def test_infer_objections_plant_manager():
    ...
    assert len(objs) > 0
    assert any(
        "team" in o["objection"].lower() or "shelf" in o["objection"].lower()
        for o in objs
    )
    ...
    objs = [
        {
            ...
            "rebuttal_hint": "Address adoption.",
        }
    ]
