...
Voice = Literal["sugarman", "murray", "hunt", "surface"]
AssetType = Literal[
    "landing_hero",
    "landing_sections",
    "email_single",
...
# ----------------------------
# Voice methodology profiles
# ----------------------------
VOICE_PROFILES: Dict[str, Dict[str, Any]] = {
    "sugarman": {
        "name": "Sugarman",
        "description": "Direct-response copywriting with psychological triggers, slippery slide scoring, and objection coverage",
        "system_guidance": (
        ...
        "tone": "confident, ops-smart, specific, psychologically persuasive",
        "run_triggers": True,
        "run_slippery": True,
        "run_objections": True,
        "run_concept": True,
    ...
    "murray": {
        "name": "Murray Brand Voice",
        "description": "Standard OTE brand voice — executive-level, factual, credible. No psychological triggers. Let the numbers speak.",
        "system_guidance": (
            "=== MURRAY BRAND VOICE ===\n"
            "1. LEAD WITH RESULTS: Open with the outcome, not the company name.\n"
            "2. METRICS FIRST: Include at least 3 specific metrics (%, $, days, units).\n"
            "3. DIRECT QUOTES: Use actual customer quotes — never paraphrase.\n"
            "4. FACTUAL TONE: Let the numbers speak. No superlatives, no hype.\n"
            "5. STRUCTURE: Challenge (what was broken) → Approach (what we did) → Result (what changed).\n"
            "6. SPECIFICITY: Name the industry, plant type, scale (# lines, # SKUs, # plants).\n"
            "7. SHORT PARAGRAPHS: 2-3 sentences max with clear subheads.\n"
            "8. NO PSYCHOLOGICAL TRIGGERS: No urgency tactics, no fear appeals, no open loops.\n"
            "9. PEER-TO-PEER: Write for the VP Ops who has 4 minutes to read.\n"
            "10. ACTIONABLE CLOSE: End with a practical next step, not a generic CTA."
        ),
        "tone": "executive-level, factual, credible, peer-to-peer",
        "run_triggers": False,
        "run_slippery": False,
    ...
    "hunt": {
        "name": "Hunt",
        "description": "Consultative challenger — lead with an insight they haven't considered, teach something new, earn the next conversation.",
        "system_guidance": (
            "=== HUNT CONSULTATIVE CHALLENGER METHOD ===\n"
            "1. LEAD WITH THE UNCOMFORTABLE TRUTH: Name the thing they know but haven't articulated.\n"
            "2. TEACH SOMETHING NEW: Every touchpoint should leave them smarter, even if they never buy.\n"
            "3. PEER-TO-PEER TONE: You're a scheduling peer who consults, not a vendor pitching.\n"
            "4. ONE INSIGHT PER MESSAGE: Don't pile on — one sharp observation beats five generic ones.\n"
            "5. SPECIFICITY OVER CLAIMS: 'Your changeover costs 14 hours/week' beats 'we improve efficiency'.\n"
            "6. DIAGNOSTIC QUESTIONS: Ask questions that reveal you understand their world.\n"
            "7. EARN THE CONVERSATION: CTA is always 'worth 15 minutes?' not 'buy now'.\n"
            "8. SHOW YOUR REPS: Reference pattern-matching from 300+ implementations.\n"
            "9. NAME THE CONSEQUENCE: Connect the insight to a business outcome they're already feeling.\n"
            "10. ADVISORY CLOSE: End with what they'd learn in a conversation, not what you'd sell them."
        ),
        "tone": "advisory, direct, insight-led, peer-to-peer",
        "run_triggers": False,
        "run_slippery": True,
        "run_objections": True,
        "run_concept": True,
    ...
    "surface": {
        "name": "Surface",
        "description": "Conversational closer — authentic, direct, no corporate polish. Reads like a message from a trusted advisor.",
        "system_guidance": (
            "=== SURFACE CONVERSATIONAL CLOSER METHOD ===\n"
            "1. WRITE LIKE YOU TALK: No corporate polish. If you wouldn't say it out loud, don't write it.\n"
            "2. SHORT SENTENCES: Most under 10 words. Fragments are fine.\n"
            "3. FIRST PERSON: 'I' not 'we'. Personal, not institutional.\n"
            "4. CASUAL CONFIDENCE: You know your stuff — no need to prove it with jargon.\n"
            "5. ONE CLEAR ASK: Every message has exactly one next step.\n"
            "6. NO FORMALITIES: Skip 'I hope this finds you well' and 'Best regards'.\n"
            "7. REFERENCE SOMETHING REAL: Their LinkedIn post, their company news, their role.\n"
            "8. BREVITY IS RESPECT: Under 100 words for messages, under 300 chars for LinkedIn.\n"
            "9. WARM BUT DIRECT: Friendly without being fake. No exclamation points.\n"
            "10. CLOSE LIKE A FRIEND: 'Worth a quick chat?' not 'Please don't hesitate to reach out'."
        ),
        "tone": "conversational, authentic, casual, direct",
        "run_triggers": False,
        "run_slippery": False,
        "run_objections": False,
        "run_concept": False,
...
# LLM prompting
# ----------------------------
def build_system_prompt(brand: Dict[str, Any], voice: Voice = "sugarman") -> str:
    identity = brand.get("identity", "")
    brand_voice = ", ".join(brand.get("voice", []))
    do_not_say = ", ".join(brand.get("do_not_say", [])) or "(none)"
    voice_profile = VOICE_PROFILES.get(voice, VOICE_PROFILES["sugarman"])

    return f"""
You are a copywriter writing for {brand['brand_name']} using the {voice_profile['name']} methodology.
{identity}

Brand voice: {brand_voice}
Methodology tone: {voice_profile['tone']}
...
async def generate_llm(req: OTECopyRequest, brand: Dict[str, Any]) -> Tuple[Dict[str, Any], str]:
    messages = [
        {"role": "system", "content": build_system_prompt(brand, req.voice)},
...
async def list_voices():
    return {
        "voices": {
            k: {"name": v["name"], "description": v["description"], "tone": v["tone"]}
...
async def generate(req: OTECopyRequest):
    ...
    voice_profile = VOICE_PROFILES.get(req.voice, VOICE_PROFILES["sugarman"])
    ...
    triggers = detect_triggers(flat_text) if voice_profile["run_triggers"] else []
    concept = extract_concept(flat_text) if voice_profile["run_concept"] else {}
    if voice_profile["run_objections"]:
        ...
        objection_analysis = []
    slide_score = slippery_score(flat_text) if voice_profile["run_slippery"] else 0.0
